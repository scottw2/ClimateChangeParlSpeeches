...
BGRz7 <- c('#0000ff', '#2f510a', '#ff0000','#b64949', '#ff8d00','#5b5b5b', '#000000')
BGRz5 = c('#0000ff', '#ff0000','#ff8d00','#5b5b5b','#000000')
...
PartyTime <- Speeches %>% 
  ...
  geom_col(alpha = 1, key_glyph = draw_key_point) +
  scale_fill_manual(values = BGRz7) +
...
ggsave(PartyTime, filename = "PartyTime7.png", dpi = 400,
...
TopicTimeParty <- SpeechesLong %>% 
  ...
  filter(Year >= 1989) %>% 
  ggplot(aes(x = Year, y = AvProbability)) +
  geom_smooth(alpha = 0.15, show.legend = F, aes(colour = partycolchart)) +
  geom_point(size = 0.5, aes(colour = partycolchart)) +
  scale_y_continuous(limits = c(0, NA), oob = squish) +
  theme_minimal() +
  ...
  facet_wrap(~Topic, scales = "free_y") +
  scale_colour_manual(values = BGRz5) +
  scale_fill_manual(values = BGRz5) +
  guides(colour = guide_legend(override.aes = list(size = 5)))

ggsave(TopicTimeParty, filename = "TopicTimeParty6.png", dpi = 400,
       width = 15, height = 10.5, units = "in", type = "cairo")
...
TopicParty <- SpeechesLong %>% 
  ...
  theme(legend.position ="bottom",
        ...
        plot.caption = element_text(family = "Open Sans", face = "italic"),
        plot.margin = unit( c(0.25,0.25,0.25,0.25), units = "in")) +
  scale_colour_manual(values = BGRz7) +
...
ggsave(TopicParty, filename = "TopicParty3.png", dpi = 400,
       width = 10, height = 7, units = "in", type = "cairo")  
...
TopMPSTopic <- TopMPS50 %>% 
  ...
  labs(title = "Which Topics Have Prominent MPs Focused on the Most?", subtitle = "Standardised average prevalence per topic over the top 50 MPs by speech count", x = "", y = "", fill = "Standardised Av. Prev.", caption = "Own analysis, data source: https://doi.org/10.5281/zenodo.4066772")

ggsave(TopMPSTopic, filename = "TopMPsTopic50x4.png", dpi = 400,
...
SpeechLenghtR <- Speeches %>% 
  ...
  geom_jitter(aes(colour = partycolchart), alpha = 0.2, size = 1, width = 0.135)  +
  geom_flat_violin(position = position_nudge(x = .2, y = 0), aes(colour = partycolchart, fill = partycolchart)) +
  stat_summary(
    fun.data = "mean_sdl",  fun.args = list(mult = 1), 
    geom = "pointrange", color = "white",size = 0.375 ,position = position_nudge(x = .275, y = 0)) +
  geom_boxplot(colour ="black", fill = NA, width = 0.135, outlier.shape = NA ) +
  scale_y_log10(breaks = c(1,10,25,50, 100, 250, 500, 1000, 2500, 5000)) +
  scale_colour_manual(values = BGRz7) +
  scale_fill_manual(values = BGRz7) +
  theme_minimal() +
  coord_flip() +
...
ggsave(SpeechLenghtR, filename = "SpeechLengthParty4.png", dpi = 400,
       width = 10, height = 7, units = "in", type = "cairo") 
